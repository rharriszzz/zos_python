...
xlc_echocmd $o -I$e/include/python3.8 -c -o _posix.o _posix.c
 */
...
int _spawnve(int mode, const char *path, const char **argvlist, const char **envlist)
{
  if (mode == _P_OVERLAY) {
    return execve(path, argvlist, envlist);
  }
  struct inheritance inherit;
  memset(&inherit, 0, sizeof(inherit)); 
  pid_t pid = spawnp(path, 0, NULL, &inherit, argvlist, envlist);
  if (pid > 0 && mode == _P_WAIT) {
    int status = 0;
    int wait_result = 0;
    while (-1 == (wait_result = waitpid(pid, &status, 0)) && errno == EINTR) ;
...
int _spawnv(int mode, const char *path, const char **argvlist)
{
  if (mode == _P_OVERLAY) {
    return execv(path, argvlist);
  }
  return _spawnve(mode, path, argvlist, environ);
}
...
static struct constdef  posix_constants_pathconf[] = {
    ...
    {"PC_ACL", _PC_ACL},
#endif
#ifdef _PC_ACL_ENTRIES_MAX
    {"PC_ACL_ENTRIES_MAX", _PC_ACL_ENTRIES_MAX},
#endif
#ifdef _PC_ASYNC_IO
...
#endif
#ifdef _SC_MMAP_MEM_MAX_NP
    {"SC_MMAP_MEM_MAX_NP", _SC_MMAP_MEM_MAX_NP},
#endif
...
#endif
#ifdef _SC_THREAD_TASKS_MAX_NP
    {"SC_THREAD_TASKS_MAX_NP", _SC_THREAD_TASKS_MAX_NP},
#endif
#ifdef _SC_THREAD_THREADS_MAX
    {"SC_THREAD_THREADS_MAX",   _SC_THREAD_THREADS_MAX},
#endif
#ifdef _SC_THREADS_MAX_NP
    {"SC_THREADS_MAX_NP", _SC_THREADS_MAX_NP},
#endif
#ifdef _SC_TIMERS
...
#define MEM(ptr,offset,name) *(char * __ptr32 * __ptr32)((char * __ptr32)(ptr)+offset)
   unsigned char *cct = MEM(MEM(MEM(0,0x10,FLCCVT),0x25C,CVTOPCTP),0x4,RMCTCCT);
   unsigned short all_cpus = *(unsigned short *)(cct+0x6E); 
   unsigned short regular_cpus = *(unsigned short *)(cct+0x70); 
...
#if 0
	stat_result_desc.fields[ST_FIRST_UNNAMED_INDEX].name = PyStructSequence_UnnamedField;
        stat_result_desc.fields[ST_FIRST_UNNAMED_INDEX+1].name = PyStructSequence_UnnamedField;
        stat_result_desc.fields[ST_FIRST_UNNAMED_INDEX+2].name = PyStructSequence_UnnamedField;
#endif
